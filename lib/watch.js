...
/**
 * Return class 
 * Allows us to preform actions on the watch later.
 */
 function FileWatch(underlyingListeners) {
   this.fileListeners = [].concat(underlyingListeners);
 }
 FileWatch.proto.close() {
   _.map(this.fileListeners,function (listener) {
     listener.close();
   });
 }
...
var handleOptions = function(origin, defaultOptions) {
  return function() {
    ...
    args[0].forEach(function(path) {
      fileListeners.push(origin.apply(null, [path].concat(args.slice(1)))); 
    });
    return new FileWatch(fileListeners);
  }
};
...
function watch(fpath, options, cb) {
  ...
  if (is.file(fpath)) {
    ...
    var fileListener = fs.watch(fpath, options, function(evt, fname) {
      if (fname) {
